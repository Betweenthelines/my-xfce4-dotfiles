#!/bin/bash
cd ~/.config/gtk-3.0/
sed -i -e 's/gtk-theme-name =.*/gtk-theme-name = Juno/g' settings.ini
sed -i -e 's/gtk-icon-theme-name=.*/gtk-icon-theme-name= Mint-Y-Dark-Teal/g' settings.ini
...
sed -i -e 's/ColorBackground=.*/ColorBackground=#182023/g' 	terminalrc
...
## alacritty
cd ~/.config/alacritty
sed -i -e 's/background:.*/background: "#182023"/g'  colors.yml
